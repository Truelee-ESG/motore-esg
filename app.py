...
def analizza_bollette(azienda, cartella, status_label):
    ...
    for root_dir, filename in file_list:
        ...
        try:
            if ext.endswith('.pdf'):
                # Utilizzo di PyMuPDF (fitz) per estrarre la prima pagina in modo efficiente
                doc = fitz.open(file_path)
                if len(doc) > 0:
                    page = doc[0]  # Consideriamo la prima pagina della bolletta
                    text = page.get_text()
                    if text:
                        ...
                        if "energia elettrica" in text_lower or "kwh" in text_lower:
                            ...
                            try:
                                # Estrazione parole con coordinate (formato PyMuPDF: x0, y0, x1, y1, word, block_no, line_no, word_no)
                                words = page.get_text("words")
                                candidates = []
                                for i, word_item in enumerate(words):
                                    w_text = word_item[4]
                                    if re.match(r'^(?:kWh|KWh|KWH)$', w_text):
                                        # Cerchiamo parole nelle vicinanze precedenti (es. il valore numerico)
                                        for j in range(max(0, i-3), i):
                                            ...
                                            if re.match(r'^\d+[\.,]?\d*$', clean_prev):
                                                ...
                                                for k in range(max(0, j-2), min(len(words), j+3)):
                                                    if re.match(r'^F[1-3]$', words[k][4], re.IGNORECASE):
                                                        is_band = True
                                                        break
                                                if not is_band:
                                                    # Calcoliamo una dimensione approssimativa basata sull'altezza della bbox
                                                    box_height = words[j][3] - words[j][1]
                                                    candidates.append({
                                                        'valore': prev_word,
                                                        'size': box_height
                                                    })
                                if candidates:
                                    ...
                                    # Fallback con regex sul testo della prima pagina
            ...
            # Gestione della somma se vengono rilevati più valori vicini
            if consumo != "Non rilevato":
                # Puliamo e normalizziamo il valore o i valori trovati
                valori_str = re.findall(r'\d+[\.,]?\d*', str(consumo))
                if len(valori_str) > 1:
                    totale_somma = 0.0
                    for v in valori_str:
                        v_clean = v.replace('.', '').replace(',', '.') if '.' in v and ',' in v else v.replace(',', '.')
                        try:
                            totale_somma += float(v_clean)
                        except ValueError:
                            pass
                    consumo = str(totale_somma)

...
def analizza_bollette(azienda, cartella, status_label):
    ...
    pdf_files = [f for f in os.listdir(cartella) if f.lower().endswith('.pdf')]
    ...
    ws.append(["Azienda", "Nome File", "Consumo Stimato (kWh)"])
    ...
    for filename in pdf_files:
        ...
        ws.append([azienda, filename, consumo])
        success_count += 1
        status_label.config(text=f"Elaborato ({success_count}/{len(pdf_files)}): {filename}")
        status_label.update()
